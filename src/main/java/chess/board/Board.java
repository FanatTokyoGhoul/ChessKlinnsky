package chess.board;

import chess.figure.Figure;
import chess.services.ChessService;
import chess.services.ColorService;
import constant.Constant;
import graph.AdjListChessDigraph;
import graph.services.GraphService;
import my.math.MathService;
import player.BotPlayer;
import player.Player;

public class Board {
    private final ColorService cs = new ColorService();


    private int counter = 0;
    private String gameName;
    private AdjListChessDigraph board;
    private Player player;
    private Player firstPlayer;
    private Player secondPlayer;
    private Maps maps = new Maps();

    public Board(String firstName, String secondName) {
        gameName = firstName + "vs" + secondName;
        firstPlayer = new BotPlayer(firstName, new Direction(Direction.UP));
        secondPlayer = new BotPlayer(secondName, new Direction(Direction.DOWN));
        player = firstPlayer;
        board = GraphService.digraphParser();
        new ChessService().createChess(firstPlayer, secondPlayer, maps);
    }

    public Board(Player firstPlayer, Player secondPlayer, int counter, String gameName) {
        this.firstPlayer = firstPlayer;
        this.secondPlayer = secondPlayer;
        this.counter = counter;
        this.gameName = gameName;
        maps.getCells().putAll(firstPlayer.getCells());
        maps.getCells().putAll(secondPlayer.getCells());
        maps.getFigures().putAll(firstPlayer.getFigures());
        maps.getFigures().putAll(secondPlayer.getFigures());
        if(MathService.isEven(counter)) {
            player = firstPlayer;
        }else {
            player = secondPlayer;
        }
        board = GraphService.digraphParser();
    }

    public Player getPlayerWhoNowWalking() {
        return player;
    }

    public Player getPlayerWhoNowNotWalking(){
        return player.equals(firstPlayer) ? secondPlayer:firstPlayer;
    }

    public void changePlayer(){
        counter++;
        player = player.equals(firstPlayer) ? secondPlayer:firstPlayer;
    }

    public AdjListChessDigraph getDigraph() {
        return board;
    }

    public Player getFirstPlayer() {
        return firstPlayer;
    }

    public Player getSecondPlayer() {
        return secondPlayer;
    }

    public Maps getMaps() {
        return maps;
    }

    public String getGameName() {
        return gameName;
    }

    public void setGameName(String gameName) {
        this.gameName = gameName;
    }

    public int getCounter() {
        return counter;
    }

    @Override
    public String toString() {

        String[] f = new String[91];

        for (int i = 0; i < f.length; i++){
            if(maps.getFigures().get(Constant.toChessCoord(i)) == null){
                f[i] = " ";
                continue;
            }
            Figure figure = maps.getFigures().get(Constant.toChessCoord(i));
            if(firstPlayer.getCells().get(figure) != null) {
                f[i] = cs.toBlue(figure.toString());
            }else {
                f[i] = cs.toRed(figure.toString());
            }
        }
        return "          11 _ 11\n" +
                "        10 _/" + f[85] + "\\_ 10\n" +
                "       9 _/" + f[84] + "\\_/" + f[86] + "\\_ 9\n" +
                "     8 _/" + f[83] + "\\_/" + f[74] + "\\_/" + f[87] + "\\_ 8\n" +
                "   7 _/" + f[82] + "\\_/" + f[73] + "\\_/" + f[75] + "\\_/" + f[88] + "\\_ 7\n" +
                "   _/" + f[81] + "\\_/" + f[72] + "\\_/" + f[63] + "\\_/" + f[76] + "\\_/" + f[89] + "\\_\n" +
                "6 /" + f[80] + "\\_/" + f[71] + "\\_/" + f[62] + "\\_/" + f[64] + "\\_/" + f[77] + "\\_/" + f[90] + "\\ 6\n" +
                "  \\_/" + f[70] + "\\_/" + f[61] + "\\_/" + f[52] + "\\_/" + f[65] + "\\_/" + f[78] + "\\_/\n" +
                "5 /" + f[69] + "\\_/" + f[60] + "\\_/" + f[51] + "\\_/" + f[53] + "\\_/" + f[66] + "\\_/" + f[79] + "\\ 5\n" +
                "  \\_/" + f[59] + "\\_/" + f[50] + "\\_/" + f[41] + "\\_/" + f[54] + "\\_/" + f[67] + "\\_/\n" +
                "4 /" + f[58] + "\\_/" + f[49] + "\\_/" + f[40] + "\\_/" + f[42] + "\\_/" + f[55] + "\\_/" + f[68] + "\\ 4\n" +
                "  \\_/" + f[48] + "\\_/" + f[39] + "\\_/" + f[30] + "\\_/" + f[43] + "\\_/" + f[56] + "\\_/\n" +
                "3 /" + f[47] + "\\_/" + f[38] + "\\_/" + f[29] + "\\_/" + f[31] + "\\_/" + f[44] + "\\_/" + f[57] + "\\ 3\n" +
                "  \\_/" + f[37] + "\\_/" + f[28] + "\\_/" + f[20] + "\\_/" + f[32] + "\\_/" + f[45] + "\\_/\n" +
                "2 /" + f[36] + "\\_/" + f[27] + "\\_/" + f[19] + "\\_/" + f[21] + "\\_/" + f[33] + "\\_/" + f[46] + "\\ 2\n" +
                "  \\_/" + f[26] + "\\_/" + f[18] + "\\_/" + f[12] + "\\_/" + f[22] + "\\_/" + f[34] + "\\_/\n" +
                "1 /" + f[25] + "\\_/" + f[17] + "\\_/" + f[11] + "\\_/" + f[13] + "\\_/" + f[23] + "\\_/" + f[35] + "\\ 1\n" +
                "  \\_/" + f[16] + "\\_/" + f[10] + "\\_/" + f[6] + "\\_/"+ f[14] + "\\_/" + f[24] + "\\_/\n" +
                "  a \\_/" + f[9] + "\\_/" + f[5] + "\\_/" + f[7] + "\\_/" + f[15] + "\\_/ l\n" +
                "    b \\_/" + f[4] + "\\_/" + f[2] + "\\_/" + f[8] + "\\_/ k\n" +
                "      c \\_/" + f[1] + "\\_/" + f[3] + "\\_/ i\n" +
                "        d \\_/" + f[0] + "\\_/ h\n" +
                "          e \\_/ g\n" +
                "             f";
    }
}
